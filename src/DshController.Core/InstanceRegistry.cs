...
namespace DshController.Core
{
    ...
    public sealed class InstanceRegistry
    {
        ...
        {
            get { return AppPaths.InstancesFile; }
        }

        public static string LegacyFilePath
        {
            get { return AppPaths.LegacyLauncherFile; }
        }
        ...
        /// <summary>按默认策略加载（含"运行中未注册实例"的自动发现）。</summary>
        public static InstanceRegistry Load()
        {
            return Load(discoverRunningInstances: true);
        }

        /// <summary>
        /// 加载实例清单。discoverRunningInstances=false 时跳过环境探测，
        /// 结果只由文件内容决定——自检/单测需要这种确定性（否则本机正在运行的
        /// 后端会被追加进清单，导致"迁移应只生成 1 个实例"之类的断言随环境漂移）。
        /// </summary>
        public static InstanceRegistry Load(bool discoverRunningInstances)
        {
            // 首次运行：把 exe 旁的旧状态复制到用户级状态目录（只复制不删除）
            AppPaths.MigrateFromExeDirIfNeeded();
            ...
            try
            {
                ...
                else if (File.Exists(LegacyFilePath))
                {
                    ...
                    if (legacy != null)
                    {
                        ...
                        {
                            // 理由: launcher.json 改名备份失败不阻断迁移，instances.json 已含全部字段，下次启动仍按它加载
                            /* 备份失败不阻断迁移（instances.json 已包含全部字段） */
                        }
            ...
            try
            {
                if (!discoverRunningInstances) throw new OperationCanceledException();   // 见下方 catch：跳过探测
            ...
            {
                // 理由: 发现失败（或被 discoverRunningInstances=false 主动跳过）不阻断启动
            }
        ...
        public void Save()
        {
            ...
            {
                var opts = new JsonSerializerOptions { WriteIndented = true };
                AppPaths.EnsureDir(Path.GetDirectoryName(FilePath));
            ...
            {
                // 理由: 保存失败不影响运行（内存态仍可用；与 legacy 行为一致）
            }
        }
