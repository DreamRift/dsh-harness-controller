...
namespace DshController.Core
{
    public static class PortTools
    {
        ...
        public static async Task<int> FindListenerPidAsync(int port)
        {
            ...
            try
            {
                ...
                using (Process p = Process.Start(psi))
                {
                    ...
                    if (!await Task.Run(() => p.WaitForExit(3000)).ConfigureAwait(false))
                    {
                        ...
                        {
                            // 理由: 超时判定的 netstat 子进程可能在 Kill 前已自行退出，Kill 抛异常无副作用；返回 0 由调用方按『未找到监听者』处理。
                        }
                        return 0;
                    }
            ...
            {
                // 理由: netstat 子进程启动/读输出可能失败（如无 netstat 或进程被拒），返回 0 视为『未找到监听者』；调用方另有端口探测与等待兜底，此处仅作尽力而为的辅助定位。
            }
            return 0;
        }
        ...
        public static async Task<bool> KillTreeAsync(int pid)
        {
            ...
            try
            {
                ...
                // 理由: 进程可能已自行退出或属其他用户而无法 Kill(entireProcessTree)，抛异常可接受；交给下方 taskkill 兜底并由 IsAlive 复核，此处吞掉即可。
            }
            ...
            if (!killed || IsAlive(pid))
            {
                ...
                {
                    // 理由: taskkill 可能未安装、权限受限或进程已回收致 Process.Start/WaitForExit 抛异常；是否真被杀死由下方 IsAlive 复核把关，此处吞掉即可。
                }
            }
