...
namespace DshController.Core
{
    ...
    public sealed class Config
    {
        ...
        public string EffectiveErrorReportDir
        {
            get
            {
                ...
                return AppPaths.DefaultReportDir;
            }
        }
        ...
        // 拆分工程后 CLI/自检在另一个程序集，且这是通用的路径净化工具（P0.4 将并入 AppPaths）
        public static string SanitizePath(string v)
        {
            if (string.IsNullOrEmpty(v) || !v.Contains("\\\\")) return v;
            try { if (Directory.Exists(v) || File.Exists(v)) return v; }
            catch
            {
                // 理由: 路径可能含非法字符或超长使 Directory.Exists/File.Exists 抛异常；视为『非现存路径』继续走反斜杠收敛，避免整条净化中断。
            }
    ...
    public sealed class JsonStringEnumConverterEx : JsonConverter<AppTheme>
    {
        public override AppTheme Read(ref Utf8JsonReader reader, Type t, JsonSerializerOptions o)
        {
            ...
            {
                // 理由: reader 当前 token 若非字符串或枚举值无法识别会抛异常；回退 AppTheme.System 保证反序列化不崩，未知主题按默认处理。
            }
