...
namespace DshController.Core
{
    ...
    public sealed class DshResolver
    {
        ...
        public static string FindOnPath(string exeName)
        {
            ...
            {
                // 理由: PATH 中可能存在超长或含非法字符的目录段，Path.Combine/File.Exists 会抛异常；返回 null 表示『PATH 上未找到』，交由调用方走下一级回退。
            }
            return null;
        }
        ...
        public static string WhereExe(string name)
        {
            try
            {
                ...
                using (Process p = Process.Start(psi))
                {
                    ...
                    if (!p.WaitForExit(3000))
                    {
                        ...
                        {
                            // 理由: where 子进程可能在超时判定与 Kill 之间已自行退出，Kill 抛异常无副作用；返回 null 即为兜底未找到，属尽力而为。
                        }
                        return null;
                    }
            ...
            {
                // 理由: cmd /c where 可能无法启动（cmd 被策略拦截/缺失）或读输出失败，属兜底查找；返回 null 由上层按命令行缺失降级，无需让异常上抛。
            }
            return null;
        }
    }
