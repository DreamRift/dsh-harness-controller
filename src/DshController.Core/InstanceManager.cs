...
namespace DshController.Core
{
    public sealed class InstanceManager : IDisposable
    {
        ...
        public InstanceManager(IUiDispatcher dispatcher, InstanceRegistry registry)
        {
            _dq = dispatcher;
            _registry = registry;
        }
        ...
        public async Task<bool> StartAsync(string id)
        {
            ...
            if (ApplyStartStamp(def, ok, DateTime.UtcNow)) _registry.Save();
            if (ok && !string.IsNullOrEmpty(def.Home) && !def.IsWsl) WriteLock(def, For(id).ChildPid);
            return ok;
        }
        ...
        /// <summary>
        /// 启动成功打点（改版·启动时间打点）：WSL 实例同样记录（实例页排序需要两类实例同轴），
        /// Windows 实例沿用「有 HOME 才打」的既有边界；失败不动旧值。返回是否写入。
        /// </summary>
        public static bool ApplyStartStamp(InstanceDef def, bool ok, DateTime nowUtc)
        {
            if (!ok || def == null) return false;
            if (def.IsWsl || !string.IsNullOrEmpty(def.Home))
            {
                def.LastStartedAt = nowUtc;
                return true;
            }
            return false;
        }
        ...
        private static void WriteLock(InstanceDef def, int pid)
        {
            ...
            {
                // 理由: 锁写入失败只失去防重复启动的双保险，不阻断本次启动；锁属非必需保护
                /* 锁写入失败不阻断启动（锁是防重复的保护，非必需） */
            }
        }
        ...
        private static void DeleteLock(InstanceDef def)
        {
            ...
            {
                // 理由: 删除锁失败仅残留陈旧锁，IsLocked 对死 PID 仍判未锁定；清理尽力而为，不阻断关闭
            }
        }
        ...
        public async Task StopAllOnExitAsync()
        {
            foreach (KeyValuePair<string, BackendManager> kv in All)
            {
                    ...
                    def.StopOnExit && kv.Value.State == BackendState.Running && kv.Value.IsMine)
                {
                    ...
                    {
                        // 理由: 单个实例停止失败不阻断其余实例退出清理，串行继续处理下一个
                    }
                }
            }
        }
        ...
        public void DisposeAll()
        {
            foreach (BackendManager m in All.Values)
            {
                try { m.Dispose(); } catch
                {
                    // 理由: 单个实例管理器释放异常不影响其余实例释放，退出清理尽力而为
                }
            }
