...
namespace DshController.Core
{
    ...
    {
        /// <summary>版本单源：由 csproj &lt;Version&gt; 经程序集特性提供（见 Diagnostics/AppVersion.cs）。</summary>
        public static string AppVersion => DshController.Core.Diagnostics.AppVersion.Current;
        ...
        private static void AppendEnvironment(StringBuilder sb)
        {
            ...
            string home = AppPaths.DefaultDshHome;
        ...
        private static string WriteReport(string fileName, string content, Config cfg)
        {
            ...
            if (string.IsNullOrEmpty(targetDir)) targetDir = AppPaths.DefaultReportDir;

            string path = TryWrite(targetDir, fileName, content);
            if (path == null)
            {
                string fallback = AppPaths.ReportsFallbackDir;
