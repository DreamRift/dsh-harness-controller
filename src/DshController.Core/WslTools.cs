    ...
    public static class WslTools
    {
            ...
            IDictionary<string, string> env = null)
        {
            ...
            try
            {
                ...
                {
                    // 理由: 进程已退出但输出管道排空等待超时，取已缓冲的 stdout/stderr 即可，结果已可用
                }
        ...
        private static void TryKill(Process proc)
        {
            ...
            {
                // 理由: 超时后杀进程失败（进程已自行退出/句柄失效）无副作用，尽力而为
            }
        }
        ...
        public static string Decode(byte[] raw)
        {
            ...
            {
                // 理由: 严格 UTF-8 解码失败（含非 UTF-8 字节）时按预期回退到 UTF-16/宽松 UTF-8，容忍新旧 wsl.exe 输出编码差异
            }
        ...
        public static async Task<bool> WriteDistroFileAsync(string distro, string linuxPath, string content, string localDir)
        {
            try
            {
                ...
                finally
                {
                    ...
                    {
                        // 理由: 清理临时上传文件失败（文件已被占用/早已删除）无副作用，残留临传文件不影响结果
                    }
                }
            }
            catch { return false; }
