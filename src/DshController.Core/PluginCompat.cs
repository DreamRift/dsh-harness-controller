...
namespace DshController.Core
{
    ...
    public static class PluginCompat
    {
        ...
        // 改版·升级治理：npm 生态存在 1/2 段版本与四段扩展，宽松形态单独解析后补齐三段
        private static readonly Regex SemverLooseRx = new Regex(
            @"^(?<v>\d+(?:\.\d+){0,2})(?:-(?<pre>[0-9A-Za-z][0-9A-Za-z.-]*))?$",
            RegexOptions.Compiled);
        ...
        private static bool TryParseSemver(string s, out int maj, out int min, out int pat, out string pre)
        {
            ...
            string core;
            if (m.Success) core = m.Groups["v"].Value;
            else
            {
                Match ml = SemverLooseRx.Match(v);
                if (!ml.Success) return false;
                core = ml.Groups["v"].Value +
                       (ml.Groups["pre"].Success ? "-" + ml.Groups["pre"].Value : "");
            }
            int dash = core.IndexOf('-');
            if (dash >= 0) { pre = core.Substring(dash + 1); core = core.Substring(0, dash); }
            string[] parts = core.Split('.');
            // 归一为三段比较（"1.2"→1.2.0、"2"→2.0.0、四段截前三段）
            if (parts.Length == 1) parts = new[] { parts[0], "0", "0" };
            else if (parts.Length == 2) parts = new[] { parts[0], parts[1], "0" };
            else if (parts.Length > 3) parts = new[] { parts[0], parts[1], parts[2] };
        ...
        private static string SearchHostRange(System.Text.Json.JsonElement root, string section)
        {
            ...
            {
                // 理由: npm 元数据只用于"支持版本"提示，取不到即按未声明处理（返回空串）
            }
            return "";
        }
    }
