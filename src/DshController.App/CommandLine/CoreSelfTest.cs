...
//                           === true，且子进程命令行含 --no-open、输出无开窗公告
//                           （需求 R4：重启绝不拉浏览器——控制器侧+子进程侧双锁）
...
namespace DshController.CommandLine
{
    internal static class CoreSelfTest
    {
        ...
        public static int Run(string[] args)
        {
            ...
            // 重构 2.0：整个自检跑在临时状态目录里——实例清单、迁移 fixture、
            // InstanceManager 的 Save 全部落到沙箱，绝不触碰用户真实 instances.json。
            string stateSandbox = Path.Combine(Path.GetTempPath(),
                "dsh-selftest-state-" + Guid.NewGuid().ToString("N").Substring(0, 8));
            try { Directory.CreateDirectory(stateSandbox); } catch { /* 理由: 创建失败下面的用例会自然报错 */ }
            string prevStateDir = AppPaths.OverrideStateDir;
            AppPaths.OverrideStateDir = stateSandbox;
            string rptDir = Path.Combine(Path.GetTempPath(), "dsh-selftest-reports");
            try { if (Directory.Exists(rptDir)) Directory.Delete(rptDir, true); } catch { /* 理由: 清理上次自检残留的临时报告目录，删除失败不影响本次自检 */ }
            string miRoot = Path.Combine(Path.GetTempPath(), "dsh-mi-test");
            try { if (Directory.Exists(miRoot)) Directory.Delete(miRoot, true); } catch { /* 理由: 清理上次自检残留的临时 miRoot，删除失败不影响本次清场 */ }
            try { Directory.CreateDirectory(Path.Combine(miRoot, "homes")); } catch { /* 理由: 预建临时 homes 目录，失败则后续 MakeTestHome 会自然报错，不在此中断 */ }
            ...
            Check(readyEvents.Count > 0 && !readyEvents[0].SuppressAutoOpen,
                "正常启动 Ready 不抑制浏览器");
            Check(!mgr.LastSpawnArguments.Contains("--no-open"),
                "正常启动子进程命令行不含 --no-open（现状保持）");
            ...
            Check(mgr.ChildPid != pid1 && mgr.ChildPid > 0, "子进程 PID 已更换",
                pid1 + " -> " + mgr.ChildPid);
            Check(readyEvents.Count > 0 && readyEvents[0].SuppressAutoOpen,
                "重启路径 Ready.SuppressAutoOpen = true（控制器侧不拉浏览器）");
            Check(mgr.LastSpawnArguments.Contains("--no-open"),
                "R4 子进程侧：重启命令行含 --no-open", mgr.LastSpawnArguments);
            bool announcedOpen = mgr.RecentOutput(2000).Any(l => l.Contains("opening the default browser"));
            Check(!announcedOpen,
                "R4 体验等价：重启期子进程输出无“opening the default browser”公告（即无新窗口）");
            ...
            else
            {
                ...
                using (Process ext = Process.Start(extPsi))
                {
                    ...
                    Check(extMgr.LastSpawnArguments.Contains("--no-open"),
                        "外部重启子进程命令行含 --no-open", extMgr.LastSpawnArguments);
            ...
            {
                // 7.0 守卫：fixture 全部写在状态沙箱内（AppPaths.OverrideStateDir）
                string instancesPath = AppPaths.InstancesFile;
                string legacyPath = AppPaths.LegacyLauncherFile;
                try
                {
                    ...
                    // discoverRunningInstances:false —— 否则本机正在运行的后端会被追加进
                    // 清单，"迁移只应生成 1 个实例"的断言就会随环境漂移（历史红条根因）。
                    var reg = InstanceRegistry.Load(discoverRunningInstances: false);
                ...
                {
                    // 沙箱内清场，供后续用例复用同一个状态目录
                    foreach (string p in new[] { instancesPath, legacyPath, legacyPath + ".v1.bak" })
                    {
                        try { if (File.Exists(p)) File.Delete(p); }
                        catch { /* 理由: 沙箱残留不影响后续断言，进程退出后随临时目录消失 */ }
                    }
                }
            ...
            // [8]-[11]：InstanceManager 成功启动会 Save registry，落点已由状态沙箱接管
            try { if (File.Exists(AppPaths.InstancesFile)) File.Delete(AppPaths.InstancesFile); }
            catch { /* 理由: 沙箱文件，删不掉也只影响本次断言的初始状态 */ }
            ...
            // ---------- 8.3) 空 home 不注入 ----------
            {
                ...
                mgrEmpty.Dispose();
                try { File.Delete(envCmd); } catch { /* 理由: 清理临时环境脚本，删除失败不影响自检结果 */ }
            }
            ...
            // 状态沙箱整体清理并还原全局覆盖
            AppPaths.OverrideStateDir = prevStateDir;
            try { if (Directory.Exists(stateSandbox)) Directory.Delete(stateSandbox, true); }
            catch { /* 理由: 临时目录清理失败不影响自检结论 */ }

            mgr.Dispose();
            try { File.Delete(failCmd); } catch { /* 理由: 清理临时失败命令脚本，删除失败不影响自检结论 */ }
        ...
        private static string MakeTestHome(string name)
        {
            ...
            try { Directory.CreateDirectory(home); } catch { /* 理由: 预建临时 HOME 目录，失败时调用方使用该路径会自然失败并计入断言 */ }
            return home;
        }
