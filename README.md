...
│ ┌ Windows 实例 · 1 个 ─┐┌ WSL 实例 · 1 个 · 1 运行中 ─┐        │ ← v0.5.0 双标签页
│ ├──────────────────────────────────────────────────────────────┤│
│ │ 实例 [主实例 · :3080 ▾]        [＋新建][⧉克隆][🗑删除]        ││
│ │ 共 1 个 Windows 实例 · 本机直接运行 · 当前环境 harness v0.1.0 ││
│ │ ⛔ 启动失败：子进程早退（主实例）        [打开报告]            ││ ← 失败提示条
│ │    报错详情 + 实例信息 + 时间已写入报告：D:\报错\…fail_….md    ││
│ │ ┌──────────────────────────────────────────────────────────┐ ││
│ │ │ ● 运行中 · 本程序启动  [WINDOWS] (harness v0.1.0-rc.7)    │ ││
│ │ │ http://127.0.0.1:3080/ [复制]                    进程 PID │ ││
...
│ │ ▾ 实例设置 · 主实例（default）                                ││
│ │    网络     主机/端口/[推荐空闲端口]                          ││
│ │    路径     工作目录/DSH_HOME（WSL 页：发行版/WSL DSH_HOME）  ││
│ │    HARNESS  版本 [跟随当前环境(v0.1.0-rc.7) ▾] [检测][拉列表] ││
│ │    高级     trusted-hosts / 停止后关闭(WSL) / 行为开关        ││
│ ├──────────────────────────────────────────────────────────────┤│
│ │ ▸ 全局设置（报告目录[浏览][打开] / 实例目录根 / 默认工作区…） ││
│ │ 🖥 控制台 [滚动：开] [复制日志] [清空] [打开报告目录]          ││
│ │ ┌──────────────────────────────────────────────────────────┐ ││
│ │ │ 21:30:01 [WIN·主实例] 已启动 dsh web（PID 18672）         │ ││
│ │ └──────────────────────────────────────────────────────────┘ ││
│ │ Windows 实例 1 个 · WSL 实例 1 个 · WIN harness v0.1.0-rc.7 …││
└────────────────────────────────────────────────────────────────┘
```
...
WSL 环境设置 `wslShutdownPolicy`（在 **WSL 实例标签页 → 实例设置 → 停止后关闭** 中配置）：
...
├── MainWindow.xaml / .cs      # 主窗口（双标签页 + 全局设置 + 共享控制台）
