...
> **Windows 与 WSL2 实例在两个独立标签页中分别管理**，支持互相隔离的多实例、
> **按实例锁定指定 harness 版本**（默认跟随当前环境主实例版本）。
...
- [快速开始](#快速开始)
- [配置说明](#配置说明)
- [harness 版本锁定（v0.5.0）](#harness-版本锁定v050)
- [多实例与隔离](#多实例与隔离)
...
- **启动失败详细报告**：失败时由核心层自动生成 Markdown 诊断报告（实例名称/ID/运行环境/
  端口/DSH_HOME/harness 版本 + 生成时间 + 环境 / dsh 解析轨迹 / 配置 / 端口状态 /
  输出转录 / 异常 / 排障建议），文件名含实例 ID 与时间戳，保存目录可自定义
...
- **harness 版本锁定（v0.5.0）**：每实例可锁定指定 harness 版本（经 `npx` 拉取
  `@deepseek-ai/dsh@<版本>` 启动）；默认**跟随当前环境主实例版本**（自动检测
  Windows 全局 dsh / WSL 发行版内 dsh 的版本）；新建实例默认填入检测到的版本，可改为任意指定版本
...
│ ┌ Windows 实例 ─┐ ┌ WSL 实例 ─┐                          │ ← v0.5.0 双标签页
│ ├────────────────────────────────────────────────────────┤ │
│ │ 实例 [主实例 ▾]                共 1 个 Windows 实例     │ │
│ │ ┌────────────────────────────────────────────────────┐ │ │
│ │ │ ● 运行中 · 本程序启动          [WINDOWS]            │ │ │
│ │ │ http://127.0.0.1:3080/ [复制]  harness v0.1.0-rc.7 │ │ │
│ │ │ DSH_HOME: C:\...\instances\default   进程 PID  18672│ │ │
│ │ └────────────────────────────────────────────────────┘ │ │
│ │  [▶ 启动] [⟳ 重启] [⏹ 停止] [🌐 打开界面]             │ │
│ │  [＋新建实例] [⧉克隆实例] [🗑删除实例]                 │ │
│ │ ▸ 实例设置（主机/端口/工作目录/DSH_HOME/harness 版本…）│ │
│ ├────────────────────────────────────────────────────────┤ │
│ │ ▸ 全局设置（报告目录 / 实例目录根 / 默认工作区…）       │ │
│ │ 🖥 控制台 [滚动：开] [清空] [打开报告目录]              │ │
│ │ ┌────────────────────────────────────────────────────┐ │ │
│ │ │ 21:30:01 [WIN·主实例] 已启动 dsh web（PID 18672）   │ │ │
│ │ └────────────────────────────────────────────────────┘ │ │
│ │ Windows 实例 1 个 · WSL 实例 1 个 · 报告目录: … · v0.5.0│ │
└──────────────────────────────────────────────────────────┘
```
  ...
  "instances": [
    {
      ...
      "harnessVersion": "",              // v0.5.0：空 = 跟随当前环境主实例版本；非空 = 锁定指定版本（npx 拉取）
...
| `harnessVersion` | 空 | harness 版本锁定（v0.5.0）：空 = 跟随当前环境主实例版本；如 `0.1.0-rc.7` = 经 npx 拉取该版本启动 |
...
每个实例可以**跟随当前环境主实例版本**（默认），也可以**锁定指定 harness 版本**启动：

- **默认（`harnessVersion` 为空）**：直接使用当前环境已安装的 dsh——
  Windows 实例用本机全局 dsh，WSL 实例用发行版内 dsh；
- **锁定（如 `harnessVersion: "0.1.0-rc.7"`）**：启动时改为执行
  `npx --yes @deepseek-ai/dsh@0.1.0-rc.7 web ...`（Windows 用本机 npx.cmd，
  WSL 用发行版内原生 npx）；npx 会拉取并缓存该版本，**首次使用需联网**，
  之后由 npm 缓存加速；
- **当前环境版本检测**：Windows 读 npm 全局包 `package.json`（回退执行
  `dsh --version`）；WSL 在发行版内执行 `dsh --version`（node 兜底）。
  实例设置的"检测当前版本"按钮可随时重新探测，状态卡实时显示生效版本
  （"环境默认"或"锁定版本"）；
- **新建实例默认版本**：新建/克隆对话框默认填入当前环境检测到的主实例版本
  （可改为任意指定版本）；克隆现有实例时默认继承源实例的版本锁定；
- 缺少 npx（或发行版内缺少 npx）而版本被锁定时，启动会失败并生成详细报告，
  提示安装 Node.js/npm 或清空版本锁定。
...
v0.5.0 起 Windows 与 WSL 实例分别在 **"Windows 实例" / "WSL 实例" 两个标签页**中管理，
各持独立实例列表、状态卡、操作按钮与实例设置，启动 / 重启 / 停止 / 打开界面 /
日志 / 设置均按各自环境选中实例操作，win 与 wsl 实例互不影响。
...
控制台同步打印报告路径，并弹窗提供"打开报告 / 打开目录"：

| 触发场景 | 报告内容侧重 |
|---|---|
| dsh 命令未找到 | 4 级查找路径逐条结果 |
| npx 未找到（版本锁定时） | 版本锁定信息 + npx 安装指引 |
...
每份报告都包含：**具体的报错信息 + 实例信息（名称/ID/运行环境/端口/DSH_HOME/harness 版本）
+ 生成时间**，文件名形如 `DshController-fail_<实例ID>_20260820_213001.md`。

报告目录默认 `我的文档\DshController\error-reports`，可在 **全局设置 → 报告目录** 中
指定任意目录；目录不可写时自动回退到 exe 旁的 `reports\`。控制台右上角的
"打开报告目录"按钮可一键定位。
...
实例锁定了 harness 版本时改为：`npx --yes @deepseek-ai/dsh@<版本> web --host ... --port ...`
（WSL 实例在发行版内执行同语义脚本）。
