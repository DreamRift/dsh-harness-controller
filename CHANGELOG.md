...
## [2.1.1] - 2026-09-08（正式版 · 档案用量工作台）

- **档案页整体升级**：总计聚合与单实例详情统一为深色用量工作台；左侧档案轨道、档案概览、五张 KPI、日期范围、模型排行和会话明细使用一致的信息层级。
- **用量趋势图**：总量与单档案共用平滑渐变曲线、数值标签避让、日期 Tooltip、竖向指示线、节点高亮、键盘选点与可展开数据表；支持日/近 7 天/周/月/自定义/全部时间，长范围按周聚合。
- **真实性能统计**：usage 分面从 DSH 会话时序折叠平均首 Token 延迟（TTFT），按会话、档案和日期持久化；旧档案缺字段保持“未采集”而不伪造 0 ms。
- **会话明细清理**：不再落档、导入、计数或展示四桶 Token 全为 0 的空会话；聚合列表显示原始会话标题开头，展开详情可查看完整标题。
- **模型目录增强**：API 预设模型行补齐图片/视频/音频三态能力；模型探针与同步保留这些明确信息。
- **发布质量**：离线断言增至 418 条；约定机检 164 文件 PASS；构建 0 警告、0 错误。
